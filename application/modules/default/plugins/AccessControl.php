...
class Default_Plugin_AccessControl extends Zend_Controller_Plugin_Abstract
{
    ...
    protected function _getAcl()
    {
        if ($this->_acl == null) {
            ...
            if ($role == 1) {
                ...
                $acl->allow('admin', 'default:index', array('index','loginpopupsave','logout','clearsessionarray','forcelogout','browserfailure','sendpassword','updatecontactnumber','getstates','getstatesnormal','getcities','getcitiesnormal','getdepartments','getpositions','gettargetcurrency','calculatedays','calculatebusinessdays','calculatecalendardays','fromdatetodate','fromdatetodateorg','validateorgheadjoiningdate','medicalclaimdates','gettimeformat','chkcurrenttime','popup','createorremoveshortcut','sessiontour','getissuingauthority','setsessionval','checkisactivestatus','updatetheme','welcome','getmultidepts','getmultiemps', 'lakshay'));

                $acl->addResource(new Zend_Acl_Resource('default:another'));
                $acl->allow('admin', 'default:another', array('index', 'hey'));

                $this->addResource($acl, 'admin', 'default:walk', ['index', 'leaves']);
            ...
            if ($role == 2) {
                ...
                $this->addResource($acl, 'hrmanager', 'default:walk', ['index', 'leaves']);
            }if ($role == 5) {
                $acl->addResource(new Zend_Acl_Resource('default:announcements'));
    ...
    private function addResource($acl, $role, $controller, $actions)
    {
